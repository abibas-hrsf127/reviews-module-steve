...
const seedUsers = () => {
  ...
  fs.writeFile(path.join(__dirname, '../../data/users.csv'), fileContent, (err) => {
      if (err) {
...
fs.writeFile(path.join(__dirname, '../../data/products.csv'), fileContent, (err) => {
    if (err) {
        console.log('error' , err)
    } else {
  ...
  const writeReviews1 = fs.createWriteStream(path.join(__dirname, '../../data/reviews1.csv'), 'utf8');
  const writeReviews2 = fs.createWriteStream(path.join(__dirname, '../../data/reviews2.csv'), 'utf8');
  const writeReviews3 = fs.createWriteStream(path.join(__dirname, '../../data/reviews3.csv'), 'utf8');
  const writeReviews4 = fs.createWriteStream(path.join(__dirname, '../../data/reviews4.csv'), 'utf8');
  const writeReviews5 = fs.createWriteStream(path.join(__dirname, '../../data/reviews5.csv'), 'utf8');
  const writeReviews6 = fs.createWriteStream(path.join(__dirname, '../../data/reviews6.csv'), 'utf8');
  const writeReviews7 = fs.createWriteStream(path.join(__dirname, '../../data/reviews7.csv'), 'utf8');
  ...
  function seedingReviews(writer, encoding, amount, callback) {
    ...
    function write() {
      ...
      do {
        ...
        var data = `${userId},${productId},${isHelpful},${isRecommended},${subject},${description},${ratingSize},${ratingQuality},${ratingWidth},${ratingComfort},${category}\n`;
